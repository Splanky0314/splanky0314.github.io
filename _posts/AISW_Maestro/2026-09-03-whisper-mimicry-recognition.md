...
(*turbo 대형조 0.17은 `아`처럼 짧은 단어가 모델의 임의 출력과 쉽게 겹쳐 높게 나온 값으로 보인다.)
...
(*소형조 turbo .74는 실제로 `안녕`을 들은 것이 아니라, turbo가 반복 출력한 `다음 영상에서 만나요`가 완화 기준에 걸려 높게 나온 값으로 보인다.)
...
- 다만, **대형조의 비표준어(슈루룽 등) 인식 부문에서는 '자기지도 임베딩+DTW 매칭(보호자 기준음성과의 유사도)'이 더 높은 성능**을 보인다.
